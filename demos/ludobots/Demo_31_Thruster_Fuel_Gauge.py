...
# AB: Most code copied from demos/robots/rocket_ship.py

sim = pyrosim.Simulator(play_blind=False, play_paused=False, debug=True,
...
for i in range(NUM_THRUSTERS):
    ...
    thrusters[i] = sim.send_thruster(side_cyls[i],
                                     x=0, y=0, z=-1,
                                     lo=0, hi=40,
                                     momentumBudget=MOMENTUM_BUDGET)

    proprio[i] = sim.send_proprioceptive_sensor(joint_id=thrusters[i])
...
for i in range(NUM_THRUSTERS):
	plt.plot(sim.get_sensor_data(proprio[i]))

plt.title('Fuel vs time')
plt.xlabel('Time')
plt.ylabel('Fuel gauge reading')
